function timeline(collection, options) {
    ...
    function setHeightandWidths(tl) {
        ...
        function setHeights() {
            ...
            const transformString = `translateY(${evenIndexTallest*2 + 40}px)`;
            tl.items.forEach((item, i) => {
                if (i % 2 === 0) {
                    item.style.height = `${evenIndexTallest*2}px`;
                    ...
                    item.style.height = `${oddIndexTallest*2}px`;
                    if (tl.settings.horizontalStartPosition !== 'bottom') {
                        item.classList.add('timeline__item--bottom');
                        addTransforms(item, transformString);
                    } else {
            ...
            tl.scroller.style.height = `${evenIndexTallest*2 + oddIndexTallest*2}px`;
        }
    ...
    function addNavigation(tl) {
        ...
        if (tl.items.length > tl.settings.visibleItems) {
            ...
            prevArrow.style.top = `${topPosition + 84}px`;
            nextArrow.style.top = `${topPosition + 84}px`;
            if (currentIndex === 0) {
    ...
    function addVerticalNavigation(tl) {
        if (tl.items.length > 4) {
            ...
            prevArrow.style.top = `${topPosition - 135}px`;
            nextArrow.style.top = `${topPosition + 600}px`;
    ...
    // Helper function to update navigation arrows' state in vertical timeline
    function updateVerticalNavigation(tl) {
        const arrowPrev = tl.timelineEl.querySelector('.timeline-nav-button--prev');
        const arrowNext = tl.timelineEl.querySelector('.timeline-nav-button--next');
        const maxIndex = tl.items.length - tl.settings.visibleItems;

        // Enable/disable navigation arrows based on currentIndex
        arrowPrev.disabled = currentIndex === 0;
        arrowNext.disabled = currentIndex === maxIndex;
    }

    function addVerticalDivider(tl) {
        const divider = tl.timelineEl.querySelector('.timeline-divider');
        ...
        verticalDivider.className = 'timeline-divider';
        verticalDivider.style.top = `-10px`;
        verticalDivider.style.bottom = `40px`;
        tl.timelineEl.appendChild(verticalDivider);
    }
